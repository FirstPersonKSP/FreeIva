...
namespace Parabox.CSG
{
    ...
    public static class CSG
    {
		...
		/// <summary>
		/// Returns a new mesh by subtracting @lhs with @rhss.
		/// </summary>
		/// <param name="lhs">The base mesh of the boolean operation.</param>
		/// <param name="rhss">The input meshes of the boolean operation.</param>
		/// <returns>A new mesh if the operation succeeds, or null if an error occurs.</returns>
		public static Model SubtractMultiple(GameObject lhs, List<GameObject> rhss)
		{
			Node a = new Node(new Model(lhs).ToPolygons());

			Node result = a;
			for (int i = 0; i < rhss.Count; i++)
            {
                result = Node.Subtract(result, new Node(new Model(rhss[i]).ToPolygons()));
			}

			List<Polygon> polygons = result.AllPolygons();
